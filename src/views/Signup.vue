<template>
	<div class="w-full flex justify-center items-center">
		<div class="w-full max-w-md p-8 space-y-3 rounded-xl dark:bg-gray-50 dark:text-gray-800">
			...
			<form class="space-y-6" @submit.prevent="createAccount" method="POST">
				<div class="space-y-1 text-sm">
					<label for="username" class="block dark:text-gray-600">Tài khoản</label>
					<input type="email" v-model="email" name="username" id="username" placeholder="Nhập tên tài khoản"
						class="w-full px-4 py-3 rounded-md dark:border-gray-300 dark:bg-gray-50 dark:text-gray-800 focus:dark:border-violet-600">
				</div>
				<div class="space-y-1 text-sm">
					<label for="password" class="block dark:text-gray-600">Mật khẩu</label>
					<input type="password" name="password" id="password" placeholder="Nhập mật khẩu" v-model="password"
						class="w-full px-4 py-3 rounded-md dark:border-gray-300 dark:bg-gray-50 dark:text-gray-800 focus:dark:border-violet-600">
					<label for="password" class="block dark:text-gray-600">Nhập lại mật khẩu</label>
				...
				<button type="submit" @click="checkLogin"
					class="block w-full p-3 text-center rounded-sm dark:text-gray-50 dark:bg-violet-600">Đăng
					ký</button>
			</form>
			<div class="flex items-center pt-4 space-x-1">
				<div class="flex-1 h-px sm:w-16 dark:bg-gray-300"></div>
				<p class="px-3 text-sm dark:text-gray-600">  nền tảng khác</p>
				<div class="flex-1 h-px sm:w-16 dark:bg-gray-300"></div>
			</div>
			...
			<p class="text-xs text-center sm:px-6 dark:text-gray-600">Chưa có tài khoản?
				<router-link :to="{ name: 'register' }" style="text-decoration: none;">
					<span class="px-8" style="color:aliceblue; font-size: 20px;">Đăng ký</span>
				</router-link>
				<a rel="noopener noreferrer" href="#" class="underline dark:text-gray-800">Đăng ký</a>
			</p>
		</div>
	</div>
</template>
