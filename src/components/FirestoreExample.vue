<template>
  <div class="body">
    <div class="container">
      ...
      <!-- Comment form -->
      <h3 class="container_heading">Thêm nhận xét của bạn về blog này</h3>
      <h3 class="container_heading underline">Nhập tên để bình luận:</h3>
...
const addComment = async () => {
  try {
    ...
    const commentData = {
      ...
      username,
      // Remove real-time timestamp, add a placeholder for now
      date: new Date()
    };
...
const commentsSorted = computed(() => comments.value.slice().sort((a, b) => b.date - a.date));
...
.button-34:hover {
  box-shadow: #000000 0 4px 8px -4px;
}
.button-34:active {
  background: #FFFF00;
  box-shadow: #000000 2px 4px -2px;
